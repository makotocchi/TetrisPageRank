...
namespace TetrisPageRank.Tests
{
    public class TestrisStackTests
    {
        ...
        public void CreateStack_ReceivesStackDigits_ReturnsCompactStack()
        {
            ...
            int compactStack = TetrisStack.CreateStack(stack[0], stack[1], stack[2], stack[3], stack[4], stack[5], stack[6], stack[7]);
        ...
        public void GetReadableDigits_ReceivesBase9Stack_ReturnsReadableDigits()
        {
            ...
            {
                TetrisStack.GetReadableDigit(compactStack, 0),
                TetrisStack.GetReadableDigit(compactStack, 1),
                TetrisStack.GetReadableDigit(compactStack, 2),
                TetrisStack.GetReadableDigit(compactStack, 3),
                TetrisStack.GetReadableDigit(compactStack, 4),
                TetrisStack.GetReadableDigit(compactStack, 5),
                TetrisStack.GetReadableDigit(compactStack, 6),
                TetrisStack.GetReadableDigit(compactStack, 7)
            };
        ...
        public void SetColumnHeight_ReceivesColumnAndHeight_ReturnsProperStack(int column, int height)
        {
            ...
            int compactStack = TetrisStack.CreateStack(stackDigits[0], stackDigits[1], stackDigits[2], stackDigits[3], stackDigits[4], stackDigits[5], stackDigits[6], stackDigits[7]);
            int[] expectedDigits = (int[])stackDigits.Clone();
            expectedDigits[column] = height;
            int expectedCompactStack = TetrisStack.CreateStack(expectedDigits[0], expectedDigits[1], expectedDigits[2], expectedDigits[3], expectedDigits[4], expectedDigits[5], expectedDigits[6], expectedDigits[7]);
