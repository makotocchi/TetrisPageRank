...
namespace TetrisPageRank.Services
{
    public class PieceJService : PieceService
    {
        ...
        public override List<TetrisDrop> GetPossibleDrops(int[] columns)
        {
            ...
            for (int i = 0; i <= 6; i++)
            {
                if (columns[i] == columns[i + 1] && columns[i] == columns[i + 2] && columns[i] <= 17)
                {
                    ...
                    possibleDrops.Add(new TetrisDrop(newColumns, 0, i, Piece.J));
                }
            }

            for (int i = 0; i <= 7; i++)
            {
                if (columns[i] == columns[i + 1] && columns[i + 1] <= 16)
                {
                    var newColumns = new int[columns.Length];
                    Array.Copy(columns, newColumns, columns.Length);
                    ...
                    possibleDrops.Add(new TetrisDrop(newColumns, 90, i, Piece.J));
                }
            }

            for (int i = 0; i <= 6; i++)
            {
                if (columns[i] == columns[i + 1] && columns[i] == columns[i + 2] + 1 && columns[i + 2] <= 17)
                {
                    var newColumns = new int[columns.Length];
                    Array.Copy(columns, newColumns, columns.Length);
                    ...
                    possibleDrops.Add(new TetrisDrop(newColumns, 180, i, Piece.J));
                }
            }

            for (int i = 0; i <= 7; i++)
            {
                if (columns[i] == columns[i + 1] - 2 && columns[i] <= 16)
                {
                    var newColumns = new int[columns.Length];
                    Array.Copy(columns, newColumns, columns.Length);
                    ...
                    possibleDrops.Add(new TetrisDrop(newColumns, 270, i, Piece.J));
                }
            }
